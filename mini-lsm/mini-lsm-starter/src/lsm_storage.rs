...
impl LsmStorageInner {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        let value = if state.memtable.get(key).is_some() {
            state.memtable.get(key)
        } else {
            state
                .imm_memtables
                .iter()
                .find(|imm_memtable| imm_memtable.get(key).is_some())
    ...
    pub fn scan(
        ...
        let state = self.state.read();
        let mut iter: Vec<Box<MemTableIterator>> = vec![];
        iter.push(Box::new(state.memtable.scan(lower, upper)));
        iter.extend(
            state
                .imm_memtables
                .iter()
                .map(|mem| Box::new(mem.scan(lower, upper))),
        );
        let merge_iterator = MergeIterator::create(iter);
